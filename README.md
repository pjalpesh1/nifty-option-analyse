...
The parser supports official NSE option-chain JSON plus flat JSON/CSV rows with expiry, strike, CE/PE type, OI, change OI, LTP, IV, Greeks, and volume.

Because GitHub Pages is static, direct browser fetch from NSE can be blocked by NSE cookies/CORS/session protection. If direct Load URL fails, use a small HTTPS NSE proxy that fetches the same NSE JSON server-side and enables CORS for this page.
